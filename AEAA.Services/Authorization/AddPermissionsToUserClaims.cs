...
namespace AEAA.Services.Authorization
{
    ...
    {
        private readonly AeaaContext _context;
        ...
        protected override async Task<ClaimsIdentity> GenerateClaimsAsync(BtUser user)
        {
            ...
            //var userdata = await _context.TUserExt.FirstOrDefaultAsync(x => x.GsuserId == userId);
            //var affiliatedata = await _context.TAffiliate.FirstOrDefaultAsync(x => x.UserextId == userdata.UserextId);
            //var affcode = affiliatedata != null ? affiliatedata.Code : "NIL";
            //identity.AddClaim(new Claim(PermissionConstants.PackedPermissionClaimType,result));
            //identity.AddClaim(new Claim(PermissionConstants.AffiliateCode, affcode));
